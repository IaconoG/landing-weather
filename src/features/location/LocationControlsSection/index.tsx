  ...
  return (
    <section className="location-controls-section" aria-label="Controles de ubicación">  
       <header className="location-controls-section__header">
        <div>
          <h3 className="location-controls-section__title">Elegi desde donde ver el clima</h3>
        </div>
        {isWeatherLoading && <span className="location-controls-section__loading">Actualizando...</span>}
      </header>
      <div className="location-controls-section__summary">
        {hasLocation ? (
          <>
            <p className="location-controls-section__summary-label">Actual</p>
            <p className="location-controls-section__summary-value">{locationLabel ?? "Ubicacion guardada"}</p>
          ...
          <>
            <p className="location-controls-section__summary-label">Sin ubicacion activa</p>
        ...
        <div className="location-controls-section__history" aria-label="Ultimas ubicaciones">
              ...
              return (
                <button
                  ...
                  className={`location-controls-section__history-item${
                    isCurrent ? " location-controls-section__history-item--current" : ""
                  }`}
                  onClick={() => handleLocationFound(location)}
                  disabled={isCurrent}
                >
                  <span className="location-controls-section__history-name">{location.label}</span>
                </button>
              );
            })}
          </div>
      ...
      <div className="location-controls-section__actions--clear">
        <LocationActionButton
          ...
          loading={false}
          label={recentLocations.length > 0 ? "Limpiar ubicaciónes" : "Limpiar ubicación"}
          variant="secondary"
        />
      </div>

      <p className="location-controls-section__hint">
        Tip: tambien podes escribir coordenadas en formato lat,lon. Ejemplo: -34.60,-58.38
      </p>
    </section>
  );
};
